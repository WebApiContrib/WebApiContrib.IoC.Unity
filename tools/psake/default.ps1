Properties {
	...
	$solution_name = "$base_dir\WebApiContrib.IoC.Unity.sln"
	$test_dll = "$build_artifacts_dir\WebApiContrib.IoC.Unity.Tests.dll"
...
Task NuGetBuild -depends Clean {
	& $nuget_exe pack "$base_dir/src/WebApiContrib.IoC.Unity/WebApiContrib.IoC.Unity.csproj" -Build -OutputDirectory $build_artifacts_dir -Verbose -Properties Configuration=Release
}
